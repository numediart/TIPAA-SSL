...
contrast_responseSchema = Schema.from_dict(
    phone_contrast_response_dict, name="contrast_response"
)

syl_contrast_responseSchema = Schema.from_dict(
    contrast_response_dict, name="syllable_contrast_response"
)
...
def kwargs_def(example):
    ...
    for prop in example:
        type_dict = {
            ...
            _io.BytesIO: fields.Raw(
                metadata={'type': 'file', 'required': True}
...
def check_target_access(
    d, target_occurence_idx=0, target_type='phone'
):  # target_type='phone',  'termination' or 'cluster'
            ...
            if d['target'] in syl:
                if target_type == 'phone':
                    idxs_of_target_occurences = [
                        i for i, p in enumerate(syl_list) if d['target'] == p
                    ]
...
def request_contrast(
    ...
    err = check_target_access(
        d, target_occurence_idx=target_occurence_idx, target_type=target_type
    )
    if err is not None:
        return Response(
            json.dumps({"status": err, "error": True}),
