...
def align_edacc(root='/home/mambauser/code/data/edacc_v1.0', split='linguistic_background', mfa_model='english_mfa', mfa_path='mfa_data',mfa_result_path='mfa_result', fs=16000, out_json='home/mambauser/code/data/align_edacc_en_train_mfa.json'):
    if os.path.exists(mfa_path):
        shutil.rmtree(mfa_path)
    if os.path.exists(mfa_result_path):
        shutil.rmtree(mfa_result_path)

    df = pd.read_csv('/home/mambauser/code/data/edacc_v1.0/test/text_edited',sep='[;,\\t]', engine='python',header=None, names=['path', 'sentence'])
