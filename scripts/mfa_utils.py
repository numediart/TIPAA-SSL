...
def get_all_phone_with_timings(
    f='data/librispeech_alignments/dev-clean/8842/304647/8842-304647-0013.TextGrid',
):
    ...
    phones = [
        [el.minTime, el.maxTime, el.mark]
        for el in tg[1]
        if el.mark not in ['sil', 'sp', '', 'spn']
    ]
...
def launch_mfa(
    ...
    textgrids_df = pd.DataFrame(
        pd.DataFrame(textgrids)
        .apply(lambda r: os.path.split(r[0])[-1].split('.')[0], axis=1)
        .tolist()
    )
...
def align_common_voice(
    root='data/cv-corpus-10.0-delta-2022-07-04/',
    lang='en',
    split='train',
    ...
    mfa_result_path='mfa_result',
    fs=16000,
    out_json='data/align_common_voice_en_train_mfa.json',
):
...
def align_synth_speech(
    ...
    mfa_result_path='mfa_result',
    fs=16000,
    out_json='data/align_synth_speech.json',
):
...
def speech_ocean_data(path='data/speechocean762'):
    ...
    scores['words_accuracy'] = scores.apply(
        lambda r: pd.DataFrame.from_records(r.words).accuracy.tolist(), axis=1
    )
    scores['words_stress'] = scores.apply(
        lambda r: pd.DataFrame.from_records(r.words).stress.tolist(), axis=1
    )
    scores['words_phones'] = scores.apply(
        lambda r: pd.DataFrame.from_records(r.words).phones.tolist(), axis=1
    )
    scores['words_total'] = scores.apply(
        lambda r: pd.DataFrame.from_records(r.words).total.tolist(), axis=1
    )
    # scores['words_text']=scores.apply(lambda r: pd.DataFrame.from_records(r.words).text.tolist(), axis=1)
    scores['words_phones_accuracy'] = scores.apply(
        lambda r: pd.DataFrame.from_records(r.words)['phones-accuracy'].tolist(), axis=1
    )
...
def align_speech_ocean(
    path='data/speechocean762',
    mfa_path='mfa_data',
    mfa_result_path='mfa_result',
    fs=16000,
):
...
def align_user_recordings(path='data/user_recordings_annotated/', fs=16000):
    ...
    target_word_indexes = df.text.apply(
        lambda r: [i for i, el in enumerate(r.split(' ')) if '*' in el]
    )
        ...
        if r['audio_path'].split('/')[-1].split('.')[0] not in failures:
            ...
            if r.target_phoneme in word_p:
                ...
                if r.target_word_indexes > 0:
                    n_phones_before_target = len(
                        ' '.join(r['phonetics'].split(' ')[: r['target_word_indexes']])
                        .replace('|', ' ')
                        .replace('-', ' ')
                        .replace('_', ' ')
                        .split(' ')
                    ...
                    n_phones_before_target = 0
                global_phoneme_index = n_phones_before_target + phoneme_index

                assert (
                    p[global_phoneme_index] == r['target_phoneme']
                ), "error: phoneme at global_phoneme_index is not the target_phone"
                ...
                LISTA__, LISTB__, LATtoHAN, HANtoLAT = words_to_unicode_chars(
                    p, phones_mfa
                )
                ...
                for i, el in enumerate(RESA):
                    if el != '-':
                        if n == global_phoneme_index:
                            global_phoneme_index_dash = i
                        n += 1

                assert (
                    RESA[global_phoneme_index_dash] == r['target_phoneme']
                ), "error: phoneme at global_phoneme_index is not the target_phone"

                global_phoneme_index_B = global_phoneme_index_dash - RESB[
                    :global_phoneme_index_dash
                ].count('-')

                if global_phoneme_index_B < len(
                    phones_mfa
                ):  # phonetization or alignment prblem
                    if (
                        phones_mfa[global_phoneme_index_B] == r['target_phoneme']
                    ):  # , "error: phoneme at global_phoneme_index is not the target_phone"
                    ...
                    # here I want to handle cases where mha phonemizer does not do the same as our ground truth phonemtics and switch to the mfa version if it's just a stress variation or a AA vs AO
                    elif unstress(phones_mfa[global_phoneme_index_B]) == unstress(
                        r['target_phoneme']
                    ):
                        ...
                        unstress(phones_mfa[global_phoneme_index_B]) in ['AA', 'AO']
                    ) and (unstress(r['target_phoneme']) in ['AA', 'AO']):
                        target_phone_indexes.append(global_phoneme_index_B)
                        status.append('success')
                        df.loc[i, 'target_phoneme'] = phones_mfa[global_phoneme_index_B]
                    else:
                        ...
                        status.append(
                            'error: phoneme at global_phoneme_index is not the target_phone, it is instead: '
                            + phones_mfa[global_phoneme_index_B]
                        )
                else:
                    target_phone_indexes.append(np.nan)
...
MAILABS_lang_dict = {'en_UK': 'en-gb', 'en_US': 'en-us', 'fr_FR': 'fr-fr', 'es_ES': 'es'}
MAILABS_lang_to_MFA_model = {
    'en_UK': 'english_mfa',
    'en_US': 'english_mfa',
    'fr_FR': 'french_mfa',
    'es_ES': 'spanish_mfa',
}
MAILABS_lang_to_MFA_dict = {
    'en_UK': 'english_uk_mfa',
    'en_US': 'english_us_mfa',
    'fr_FR': 'french_mfa',
    'es_ES': 'spanish_mfa',
}
MAILABS_lang_to_CMU_model = {'en_UK': 'english_us_arpa', 'en_US': 'english_us_arpa'}
...
def align_MAILABS(path="data/MAILABS", lang_code="en_US", phone_set='MFA_IPA', n=10000):
    ...
    if not os.path.exists(mfa_result_path):
        os.makedirs(mfa_result_path)
    ...
    df.to_csv(
        path + '/' + 'MAILABS_shuffled_aligned-' + lang_code + '_' + phone_set + '.csv'
    )
    return df, failures
...
def use_tests():
    ...
    align_common_voice(
        ...
        lang='en',
        split='dev',
        out_json='data/align_common_voice_en_dev_mfa.json',
    )
    align_common_voice(
        root='data/cv-corpus-10.0-delta-2022-07-04/',
        lang='en',
        split='test',
        out_json='data/align_common_voice_en_test_mfa.json',
    )
    ...
    align_common_voice(
        ...
        out_json='data/align_common_voice_en_train_cmu.json',
    )
    align_common_voice(
        root='data/cv-corpus-10.0-delta-2022-07-04/',
        lang='en',
        split='dev',
        mfa_model='english_us_arpa',
        out_json='data/align_common_voice_en_dev_cmu.json',
    )
    align_common_voice(
        root='data/cv-corpus-10.0-delta-2022-07-04/',
        lang='en',
        split='test',
        mfa_model='english_us_arpa',
        out_json='data/align_common_voice_en_test_cmu.json',
    )

    df, failures = align_MAILABS(
    ...
    phones_2 = df.apply(
        lambda r: remove_stress_annots(r.phone_df.cmu_phone.tolist()), axis=1
    )
