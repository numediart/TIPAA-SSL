...
def lemmatize_from_df(df):
    ...
    w_list = sum_list(
        df.apply(lambda r: remove_special_characters(r.text, lowercase=False), axis=1)
...
def use_tests():
    ...
    df_phrases['n_text'] = df_phrases.apply(
        lambda r: remove_special_characters(
            r.text, chars_to_ignore_regex='[\,\?\.\!\;\:\"\']', lowercase=True
        ),
        axis=1,
    )
    df_content['n_text'] = df_content.apply(
        lambda r: remove_special_characters(
            r.sentence, chars_to_ignore_regex='[\,\?\.\!\;\:\"\']', lowercase=True
        ),
        axis=1,
    )
    ...
    df_content[~df_content.n_text.isin(set(df_phrases.n_text.unique()))][
        ['id', 'sentence']
    ].to_csv(no_duplicates_path)
    ...
    df['n_text'] = df.apply(
        lambda r: remove_special_characters(
            r.text, chars_to_ignore_regex='[\*]', lowercase=False
        ),
        axis=1,
    )
    ...
    for i, r in df_audio_names.iterrows():
        n_text = remove_special_characters(
            r.text, chars_to_ignore_regex='[\*]', lowercase=False
        )
