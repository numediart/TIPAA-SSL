...
def explore_model_parameters(model):
    ...
    module_types_nn = {
        ...
        if (m in torch.nn.__dict__ and m != 'ModuleList')
    }
    ...
    distrib_df_squared = pd.DataFrame(
        np.histogram(weights_matrices_squared_sum, bins=20)
    ).T

    distrib_df_values = pd.DataFrame(np.histogram(all_weights, bins=100)).T
    distrib_df_values[
        int(len(distrib_df_values) / 2 - 10) : int(len(distrib_df_values) / 2 + 10)
    ]
