...
df = pd.read_csv('data/EDACC/edacc_v1.0/test/segments_copy', sep=' ', header=None, names=['EDACC', 'WAV_File', 'Start_Time', 'Stop_Time'])
...
for index, row in df.iterrows():
    ...
    output_filename = os.path.join(output_folder, f"{os.path.splitext(wav_file)[0]}-0{segment_number}.wav")
    ...
    print(f'Split {wav_file} - Start: {start_time}, Stop: {stop_time}, Saved: {output_filename}')
