...
from src.load_data import load_dataset_MAILABS, build_df_all_frames, build_df_all_phoneme_instances, load_dataset_commonvoice
from src.text_processing import remove_stress_annots

def build_frame_dataset(unstressed=False, outname='df_all_frames_MAILABS_train_n_examples_1000', MAILABS_path='/mnt/c/Users/noe_t/OneDrive - UMONS/flowchase/datasets/MAILABS'):
    ...
    df_all_instances_select = df_all_instances.sample(frac=1, random_state=0).groupby('unstressed_phoneme').apply(lambda x: x.sample(n=min(len(x), 500))).reset_index(drop=True)
    df_all_instances_select_stressed = df_all_instances.sample(frac=1, random_state=0).groupby('phoneme').apply(lambda x: x.sample(n=min(len(x), 500))).reset_index(drop=True)
    ...
    if unstressed:
        df_all_instances_select_no_CH_JH=df_all_instances_select[(df_all_instances_select.phoneme!="CH")&(df_all_instances_select.phoneme!="JH")]
        df_all_instances_select_no_CH_JH.phoneme=df_all_instances_select_no_CH_JH.unstressed_phoneme
        return df_all_instances_select_no_CH_JH
    else:
        df_all_instances_select_stressed_no_CH_JH=df_all_instances_select_stressed[(df_all_instances_select_stressed.phoneme!="CH")&(df_all_instances_select_stressed.phoneme!="JH")]
...
def fit_and_score_classifiers(df_all_instances_train, df_all_instances_test):
    ...
    my_dict={p:len(df_all_instances_train[df_all_instances_train.phoneme==p]) for p in phoneme_set}
    ...
    for clf in classifiers:
        ...
        no_stress_score=sum([el1==el2 for el1,el2 in zip(remove_stress_annots(df[0]),remove_stress_annots(df[1]))])/len(df)
    ...
    # performance after stress removal
    sum([el1==el2 for el1,el2 in zip(remove_stress_annots(df[0]),remove_stress_annots(df[1]))])/len(df)
