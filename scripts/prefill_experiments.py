...
def prefill_content_variations(sentences):
    ...
    df_MFA_IPA_GB_syl_mis=df_MFA_IPA_GB[df_MFA_IPA_GB.n_syl_mismatches.str.len()!=0][["segmented_text","phonetics"]]
    df_MFA_IPA_US_syl_mis=df_MFA_IPA_US[df_MFA_IPA_US.n_syl_mismatches.str.len()!=0][["segmented_text","phonetics"]]
    ...
    syl_inconsistencies_CMU=df_CMU[df_CMU.n_syl_mismatches.str.len()!=0].apply(lambda r: (r.segmented_text.split(' ')[r.n_syl_mismatches[0]],r.phonetics.split(' ')[r.n_syl_mismatches[0]]), axis=1)
    syl_inconsistencies_MFA_GB=df_MFA_IPA_GB[df_MFA_IPA_GB.n_syl_mismatches.str.len()!=0].apply(lambda r: (r.segmented_text.split(' ')[r.n_syl_mismatches[0]],r.phonetics.split(' ')[r.n_syl_mismatches[0]]), axis=1)
    syl_inconsistencies_MFA_US=df_MFA_IPA_US[df_MFA_IPA_US.n_syl_mismatches.str.len()!=0].apply(lambda r: (r.segmented_text.split(' ')[r.n_syl_mismatches[0]],r.phonetics.split(' ')[r.n_syl_mismatches[0]]), axis=1)
...
def prefill_words():
    from src.pronunciation_dictionaries import get_augmented_mfa_dict, lang_to_MFA_g2p_models, get_mfa_dict
    ...
    for lang in langs:
        ...
        df_MFA_IPA_syl_mis=df_MFA_IPA[df_MFA_IPA.n_syl_mismatches.str.len()!=0][["segmented_text","phonetics","n_syl_mismatches"]]
        ...
        syl_inconsistencies_MFA=df_MFA_IPA_syl_mis.apply(lambda r: (r.segmented_text.split(' ')[r.n_syl_mismatches[0]],r.phonetics.split(' ')[r.n_syl_mismatches[0]]), axis=1)
        ...
        # lang
        tqdm.pandas()
        df_MFA_IPA['segmented_text_SSP_text']=df_MFA_IPA.progress_apply(lambda r: syllabified_text(r.text, n_vowels(r.text, lang=lang), pd.DataFrame(columns=['n_syls', 'n_syls_SonoriPy', 'normalized_text', 'syllables']), lang, combined_sonoripy_dtw=False)[0], axis=1)
        df_MFA_IPA['segmented_text_SSP_DTW']=df_MFA_IPA.progress_apply(lambda r: syllabified_text(r.text, n_vowels(r.text, lang=lang), pd.DataFrame(columns=['n_syls', 'n_syls_SonoriPy', 'normalized_text', 'syllables']), lang)[0], axis=1)
        df_MFA_IPA['segmented_text_lookup_SSP_DTW']=df_MFA_IPA.progress_apply(lambda r: syllabified_text(r.text, n_vowels(r.text, lang=lang), syllables_dfs[lang], lang)[0], axis=1)
...
def prefill_cmuarctic():
    ...
    for lang in langs:
        ...
        df_MFA_IPA['segmented_text_lookup_SSP_DTW']=df_MFA_IPA.progress_apply(lambda r: ' '.join([syllabified_text(word, n_vowels(word, lang=lang), syllables_dfs[lang], lang)[0] for word in r.segmented_text.replace('|','').split(' ')]), axis=1)
        c="segmented_text_lookup_SSP_DTW"

        df_MFA_IPA['n_syl_mismatches_best']=df_MFA_IPA.apply(lambda r: [ i for i,(el1,el2) in enumerate(zip(r.phonetics.split(' '),r.segmented_text.split(' '))) if el1.count('|')!=el2.count('|')], axis=1)
        
        df_MFA_IPA_syl_mis=df_MFA_IPA[df_MFA_IPA.n_syl_mismatches_best.str.len()!=0][["segmented_text","phonetics","n_syl_mismatches_best"]]
        syl_inconsistencies_MFA=df_MFA_IPA_syl_mis.apply(lambda r: (r.segmented_text.split(' ')[r.n_syl_mismatches_best[0]],r.phonetics.split(' ')[r.n_syl_mismatches_best[0]]), axis=1)

        df_MFA_IPA['n_syls']=df_MFA_IPA.apply(lambda r: [ el.count('|')+1 for el in r.phonetics.split(' ') ], axis=1)
    ...
    inventory_dict2 = dict(Counter(df_MFA_IPA_dict.dropna().apply(lambda r:r.phonetics.count('|')+1, axis=1).tolist()))
...
def prefill_mls():
    ...
    for k in dfs:
        if k.split('/')[0] in to_lang:
            ...
            print(k, len(df))
            df_MFA_IPA,df_errors=prefill_content(sentences, lang=lang, mode='MFA_IPA')
            df_MFA_IPA.to_csv('prefill_MFA_IPA_'+k.replace('/','_')+'_'+lang+'.csv')

    for k in dfs:
        if k.split('/')[0] in to_lang:
            lang=to_lang[k.split('/')[0]]
            df_MFA_IPA=pd.read_csv('prefill_MFA_IPA_'+k.replace('/','_')+'_'+lang+'.csv')
            import ast
            df_MFA_IPA.n_syl_mismatches=df_MFA_IPA.n_syl_mismatches.apply(ast.literal_eval)
            # mismatch syllables between text and phonetics, for each phonetics variation
            # df_CMU_mis=df_CMU[df_CMU.n_syl_mismatches.str.len()!=0][["segmented_text","phonetics"]]
