...
def parse_html(path='data/Focus_words_introduction_1___48.html'):
    ...
    phrases = [
        el.strip('\t').strip('</span>').replace('</span>', '')
    ...
    for phrase in phrases:
        ...
        filename = (
            remove_special_characters(filename).encode('ascii', 'ignore').decode('ascii')
            + '.json'
        )
...
def validate_json(
    path="data/Tutorials 1.0/JSON Final -ED/page100/when_talking_about_the_past_learners_of_english_often_forget_to_pronounce_the_-ed_ending_of_the_verbs_they_use.json",
):
    with open(path, 'r') as file:
        text = file.read()
    text = (
        text.replace('uuidv4()', '"uuidv4()"')
