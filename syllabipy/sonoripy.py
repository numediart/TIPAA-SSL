...
def define_categories(mode='CMU'):
    if mode == 'CMU':
        ...
        with open('data/mfa_phones.json', 'r') as openfile:
            mfa_phones = json.load(openfile)
        # https://mfa-models.readthedocs.io/en/refactor/mfa_phone_set.html
        cats = {}
        cats['vowels'] = str_to_list_of_char(
            'aeiouyàáâäæãåāèéêëēėęîïíīįìôöòóœøōõûüùúūůÿ'
        ) + str_to_list_of_char('ɝəaɔʌãeéẽɛøoõiu')
        cats['vowels'] += [
            'ɐ',
            'ɑ',
            'ɑː',
        ...
        cats['vowels'] += [
            "ᵻ"
        ]  # discovered this one for espeak in final -ed when uning en_US.
...
def SonoriPy(word, mode='CMU'):
    ...
    def no_syll_no_vowel(ss):
        ...
        for syll in ss:
            ...
            if not any(
                ((char.lower() in vowels) or (char.lower()[0] in vowels)) for char in syll
            ):
                if len(nss) == 0:
                    front += syll
                else:
    ...
    if mode != 'letters':
        ...
        for i, el in enumerate(word):
            ...
            # I need this for espeak
            # 'start'-> 's_t_ˈɑːɹ_t',   'engineer' -> "ˌɛ_n_dʒ_ɪ_n_ˈɪɹ"
            if (el.lower() in vowels) or (
                el.lower()[0] in vowels
            ):  # and el[1].lower() in vowel_ending_in_consonant:
