...
def apply_filters(df, filters_df):
    for i, filter in filters_df.iterrows():
        mask = df[filter['Column']].str.__getattribute__(filter['Operation'])(
            filter['Value']
        )
...
def main(filters_df):
    ...
    if check_password():
        ...
        num_rows = st.number_input(
            'Number of filters to add', min_value=1, max_value=10, value=1
        )
        ...
        for i in range(num_rows):
            ...
            with a:
                new_row['Column'] = st.selectbox(
                    f'Column (row {i+1})', ['text', 'formatted_phonetics']
                )
            with b:
                new_row['does/does not'] = st.selectbox(
                    f'does/does not (row {i+1})', ['does', 'does not']
                )
            with c:
                new_row['Operation'] = st.selectbox(
                    f'Operation (row {i+1})', ['startswith', 'endswith', 'contains']
                )
            with d:
                new_row['Value'] = st.text_input(f'Value (row {i+1})')
            filters_df = pd.concat(
                [filters_df, pd.DataFrame([new_row])], ignore_index=True
            )
