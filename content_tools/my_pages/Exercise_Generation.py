...
def make_exercise():
    ...
    if check_password():
        ...
        format = st.selectbox('',['Please Select Level inside Learning Program','Exercise','Activity','Activity Set','Module'],)
        ...
        if format != 'Please Select Level inside Learning Program':
            ...
            ('Options...','Count Syllables', 'Input Stress Pattern', 'Match Audio to Stress Pattern',
            'Match Word to Audio','Match Word to Stress Pattern','Pick Stress Pattern',
            'Pick Stressed Syllable','Pick Phonetics','Pick Audio Unlike Others','SpokenCard',
            'Pick Stressed Word','Pick Meaning of Audio','Match Audio to Meaning','SpokenSentence'
            ))
            if exercise_type != 'Options...':
                # Some exercise types focus on a certain pronunciation aspect, others don't
                if exercise_type in ['Match Word to Audio','Pick Phonetics','Pick Audio Unlike Others','SpokenCard','SpokenSentence']:
                    if exercise_type == 'Match Word to Audio':
                        pronunciation_aspect = st.selectbox('What pronunciation aspect would you like your exercises to be based on?',('Options...','VC1','VC2','VC3','VC4','-ED'))
                    elif exercise_type == 'Pick Phonetics':
                        pronunciation_aspect = st.selectbox('What pronunciation aspect would you like your exercises to be based on?',('Options...','VC1','VC2','VC3','VC4','-ED','TH','-S'))
                    elif exercise_type == 'Pick Audio Unlike Others':
                        pronunciation_aspect = st.selectbox('What pronunciation aspect would you like your exercises to be based on?',('Options...','VC1','VC2','-ED'))
                ...
                if pronunciation_aspect != 'Options...':
                    ...
                    if n_exercises != 0:
                        ...
                        if exercise_type != 'Match Word to Audio':
                            topic = st.text_input('What topic would you like your exercises to be based on?')
                        else:
                            topic = 'no topic'
                        if topic != '':
                            final_button = st.button('Generate ' + format) 
                            if final_button:
                                if exercise_type in ['Match Audio to Stress Pattern','Match Word to Stress Pattern']:
                                    st.write('Your exercise might take a while to generate. Please be patient.')
                                # With all the collected information, the main function in exercice.py will take over
                                ex.generate_exercise(exercise_type,pronunciation_aspect,n_exercises,topic)
                                ...
                                d_exercise = {} # creating a dictionary to follow the same structure as JSON Editor (cf. Gianie's work)
                                d_exercise['Pronunciation aspect'] = pronunciation_aspect
                                if exercise_type == 'SpokenCard': # SpokenCard and SpokenSentence need to be adapted differently to be compatible with Gianie's interface
                                    d_exercise['Exercise type'] = 'spoken_card'
                                elif exercise_type == 'SpokenSentence':
                                    d_exercise['Exercise type'] = 'spoken_sentence'
                                else:
                                    d_exercise['Exercise type'] = (re.sub('\s','_',exercise_type.lower()))
                                if exercise_type == 'SpokenCard': # SpokenCard and SpokenSentence need to be adapted differently to be compatible with Gianie's interface
                                    d_exercise['content_spoken_card'] = liste
                                elif exercise_type == 'SpokenSentence':
                                    d_exercise['content_spoken_sentence'] = liste
                                else:
                                    d_exercise['content_' + (re.sub('\s','_',exercise_type.lower()))] = liste
                                ...
                                if format == 'Exercise': # level 1 (exercise)
                                    ...
                                    if exercise_type in ['SpokenCard','SpokenSentence']:
                                        ...
                                        d_activity_title['Listening activity'] = [d_exercise]
                                ...
                                if len(liste) == 0:
                                    st.write('Sorry, no exercises were generated... Please try again.')
                                # else:
