    ...
    for module in content:
        ...
        for activity_set in module[param_to_name['activity_set']]:
            ...
            for activity in activity_set[param_to_name['activity']]:
                ...
                k = (
                    'Speaking activity'
                    if 'Speaking activity' in activity.keys()
                ...
                for exercise in activity[k]:
                    ...
                    if 'Listening' in k:
                        d['pronunciation_aspect'] = exercise[
                            param_to_name['pronunciation_aspect']
                        ]
...
def df_to_nested_dict(df):
    ...
    for m_g, m in module_groups:
        m_d = {}
        m_title = m_g
        activity_set_groups = list(
            m.groupby(['module_title', 'activity_set_title'], sort=False)
        )
        activity_sets = []
        for act_set_g, act_set in activity_set_groups:
            ...
            activity_groups = list(
                act_set.groupby(
                    ['module_title', 'activity_set_title', 'activity_title'], sort=False
                )
            )
            activities = []
            for act_g, act in activity_groups:
                ...
                for ex_g, exs in exercise_groups:
                    ...
                    if ex_g[-1] in ['spoken_card', 'spoken_sentence']:
                        ...
                        exs['phrase_data'] = exs.apply(
                            lambda r: {'pronunciation_aspect': r.pronunciation_aspect}
                            | r.phrase_data,
                            axis=1,
                        )
                    ...
                    if ex_g[-1] not in ['spoken_card', 'spoken_sentence']:
                        d[param_to_name['pronunciation_aspect']] = exs[
                ...
                act_d[
                    exs['activity_type'].iloc[0].split(' ')[0] + ' activity'
                ] = exercises
...
def process_content(df):
    ...
    all_sentences = df.phrase_data.apply(
        lambda r: [r[k] for k in r if k not in not_content_keys]
    ).sum()

    # make a string containing infos of all columns except phrase data for making ids
    L = df.apply(
        lambda r: [
            '__'.join([remove_special_characters(el).replace(' ', '_') for el in r[:-1]])
            for k in r.phrase_data
            if k not in not_content_keys
        ],
        axis=1,
    ).sum()
    ...
    ids = L_c_df.apply(
        lambda r: [el + '_' + str(i) for i, el in enumerate([r[0]] * r[1])], axis=1
    ).sum()
...
def remove_duplicates(db_content, all_phrases_df):
    all_phrases_df[
        all_phrases_df.apply(
            lambda r: r['sentence'] in db_content.sentence.tolist(), axis=1
        )
    ]

    len(all_phrases_df)
    all_phrases_df_drop = all_phrases_df.iloc[
        all_phrases_df.sentence.drop_duplicates().index
    ]
    len(all_phrases_df_drop)
    all_phrases_df_drop_db = all_phrases_df_drop[
        ~all_phrases_df_drop.apply(
            lambda r: r['sentence'] in db_content.sentence.tolist(), axis=1
        )
    ]
...
def use_tests():
    ...
    df.phrase_data = df.phrase_data.apply(
        lambda r: {properties_phrase_data_to_correct[k.lower().strip()]: r[k] for k in r}
    )
    ...
    df.phrase_data = df.phrase_data.apply(
        lambda r: {
            k.lower().strip(): r[k].strip() for k in r if k not in ['pos', 'phrase_raw']
        }
    )
    ...
    df_star = df[
        df.phrase_data.apply(
            lambda r: re.match("^\*[a-z]*\*$", r["target_content"])
            if "target_content" in r
            else False
        ).astype(bool)
    ]
    df.loc[
        ...
        'phrase_data',
    ] = df_star.phrase_data.apply(lambda r: {k: r[k].replace('*', '') for k in r})
    ...
    df.exercise_type = df.exercise_type.apply(
        lambda r: r.lower().strip().replace(' ', '_')
    )

    modules = df_to_nested_dict(df)

    with open("scripts/GE_S_data_entry.json", 'w') as f:
        json.dump(modules, f, indent=2)

    with open("scripts/PE_data_entry_manually_corrected.json", 'r') as f:
        content = json.load(f)
    ...
    with open("scripts/marie_program_modified.json", 'r') as f:
        content = json.load(f)
    ...
    with open("scripts/data_entry_output_ex.json", 'r') as f:
        content = json.load(f)

    df = nested_dict_to_df(content)
    modules = df_to_nested_dict(df)
    with open("scripts/data_entry_conversion_ex.json", 'w') as f:
        json.dump(modules, f, indent=2)
    ...
    BE.loc[
        BE.exercise_type == 'pick_stressed_pattern', 'exercise_type'
    ] = "pick_stress_pattern"

    select_wrong_ex_type = BE.loc[
        BE.exercise_type == 'spoken_sentence', 'phrase_data'
    ].apply(lambda r: list(r.keys()) != ['question', 'answer'])

    BE.loc[BE.exercise_type == 'spoken_sentence'].loc[select_wrong_ex_type]

    modules = df_to_nested_dict(BE)

    with open("scripts/PE_data_entry.json", 'w') as f:
        json.dump(modules, f, indent=2)
    ...
    speaking_types = ["Spoken card", "Spoken sentence"]
    ...
    exercise_type_to_payload = {
        ...
        "spoken_sentence": [
            "question",
            "answer",
        ],  # , (stress_category), (pronunciation_aspect)
    }
