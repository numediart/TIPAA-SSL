...
def remove_numbering(liste):
    newlist = []
    for el in liste:
        newlist.append(re.sub("[0-9]+[.)]\s*","",el.strip())) # removes the numbering in front of the list element
    return newlist
...
def create_syllabus_tree(topic="biology"):
    ...
    module_to_activity_set={module:[] for module in df["Module (week)"].dropna().unique().tolist()}
    ...
    for key, values in module_to_activity_set.items():  module_to_activity_set_str += key + ": " + ", ".join(values) + "\n"
    
    activity_set_to_activities={}
    for module in module_to_activity_set:
        for act_set in  module_to_activity_set[module]:
            activity_set_to_activities[module+", "+act_set]=df[df["Activity set"]==act_set]["Activity"].dropna().tolist()
    
    activity_set_to_activities_str = ""
    for key, values in activity_set_to_activities.items():  activity_set_to_activities_str += key + ": " + ", ".join(values) + "\n"

    activity_set_to_activities_list=[key + ": " + ", ".join(values) for key, values in activity_set_to_activities.items()]
...
def create_title(d, topic="biology"):
    ...
    if list(d.keys())[0] == 'Module title':
        ...
        prompt = """Here are example of titles for exercise modules from a learning program for English relating to the topic of "Legal profession":\n""" + data_module + "\nGenerate 5 exercise module titles of the same kind, for a learning program for English relating to the topic of " + topic #fillers[pronunciation_aspect]
