...
def phonetics_for_row(row, libri_words_df):
    ...
    for w_idx, w in enumerate(sentence.split(' ')):
        try:
            phonetics.append(
                libri_words_df[
                    (libri_words_df.file_idx == row.file_idx)
                    & (libri_words_df.word_idx == w_idx)
                    & (libri_words_df.word == w)
                ]
                .iloc[0, :]
                .phones
            )
    ...
    for f_idx, f in tqdm(enumerate(files)):
        ...
        for i, el in enumerate(tg_words):
            ...
            wav_path = os.path.join(
                audio_path, '/'.join(f.split('/')[2:]).split('.')[0] + '.flac'
            )
            d = {
                'word': word,
                'phones': " ".join(phones),
...
def libri_phonetics(
    ...
    for f in files:
        wav_path = os.path.join(
            audio_path, '/'.join(f.split('/')[-4:]).split('.')[0] + '.flac'
        )
        ...
        d = {
            'text': text,
            'phones': ' '.join(phone_df['phone'].tolist()),
            'path': f,
            'wav_path': wav_path,
        }
        d_with_timings = {
    ...
    libri_phonetics_df_with_timings.to_json(
        'data/libri_text_ipa_timings_' + data_set + '.json'
    )
...
def libri_phonetics_data(
    ...
    if os.path.exists(path_df_t) and os.path.exists(path_df):
        ...
        libri_phonetics_df_with_timings, libri_phonetics_df = libri_phonetics(
            data_set=data_set
        )
...
def learning_content(selection, n=20):
    ...
    frequent_selection_unique['frequency among examples'] = (
        counts[frequent_selection_unique.word].values / len(selection) * 100
    )
    return frequent_selection_unique[
        ['word', 'phones', 'frequency among examples']
    ].sort_values('frequency among examples', ascending=False)
...
def frequent_selection_containing(
    libri_words_df, phones=['AO0', 'IY1'], letters=[], n=20, option="contains"
):
    ...
    for phone in phones:
        selection = select_libri(selection, phone, option=option)
    for l in letters:
        selection = select_libri(selection, l, 'word', option=option)
    ...
    if phones != []:
        filename += '_phones_' + '_'.join(phones)
    if letters != []:
        filename += '_letters_' + '_'.join(letters)
    filename += '_n_' + str(n)
...
def frequent_word_selections_for_phones(
    libri_words_df,
    phones=[
        'IH0',
        'IH1',
...
def get_phone_termination_dict():
    ...
    others = [
        p
        for p in consonants
        if (p[-1][0] != 'nasal' and p[-1][0] != 'liquid' and p[-1][0] != 'semivowel')
    ]
    ...
    for p, vuv in vuv_dict.items():
        phone_termination_dict[p] = vuv_termination_dict[vuv]
...
def frequent_ed_word_selections_by_termation(libri_words_df, n=200):
    ...
    good_preterminations = [
        k for k in res['performances'] if res['performances'][k] >= 0.8
    ]
    ...
    for term in terminations:
        preterm_selection = [
            pret for pret in good_preterminations if phone_termination_dict[pret] == term
        ]
        dfs = []
        for preterm in preterm_selection:
            full_termination = ' ' + ' '.join([preterm, term])
            dfs.append(
                frequent_selection_containing(
                    libri_words_df,
                    phones=[full_termination],
                    letters=['ed'],
                    option='endswith',
                    n=n,
                ).word
...
def use_tests():
    ...
    libri_words_df[
        libri_words_df.phones.str.endswith(' S') & libri_words_df.word.str.endswith('s')
    ]
    ...
    libri_words_df[
        libri_words_df.phones.str.contains('AO0')
        & libri_words_df.phones.str.contains('IY1')
    ]
    libri_words_df[libri_words_df.phones.str.contains('OW1')]
    libri_words_df[
        libri_words_df.phones.str.endswith(' B D')
        & libri_words_df.word.str.endswith('bed')
    ]
    libri_words_df[
        libri_words_df.phones.str.endswith(' D') & libri_words_df.word.str.endswith('ied')
    ]

    libri_words_df[
        libri_words_df.phones.str.endswith(' P T')
        & libri_words_df.word.str.endswith('ped')
    ]
    libri_words_df[
        libri_words_df.phones.str.endswith(' K T')
        & libri_words_df.word.str.endswith('ked')
    ]
    ...
    libri_words_df[
        libri_words_df.phones.str.contains(' IY1 ')
        & libri_words_df.word.str.contains('ea')
    ]
    libri_words_df[
        libri_words_df.phones.str.contains(' UH1 ')
        & libri_words_df.word.str.contains('oo')
    ]
    libri_words_df[
        libri_words_df.phones.str.contains(' UW1 ')
        & libri_words_df.word.str.contains('oo')
    ]
    libri_words_df[
        libri_words_df.phones.str.contains(' AW1 ')
        & libri_words_df.word.str.contains('ou')
    ]
    libri_words_df[
        libri_words_df.phones.str.contains(' UW1 ')
        & libri_words_df.word.str.contains('ou')
