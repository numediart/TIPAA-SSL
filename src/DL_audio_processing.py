...
def speech_enhancement(s):
    ...
    enhanced = enhance_model.enhance_batch(s, lengths=torch.tensor([1.0]))
...
if False:
    ...
    def run_VAD(s):
        prob = VAD_model.get_speech_prob_chunk(
            torch.from_numpy(s.astype(np.float32))[None], torch.tensor([1.0])
        )
        return prob
    ...
    def asr(s):
        # Add relative length tensor
        texts, ns = asr_model.transcribe_batch(
            torch.from_numpy(s.astype(np.float32))[None], torch.tensor([1.0])
        )
        return texts, ns
    ...
    def speech_separation(s):
        ...
        est_sources = separator_model.separate_batch(
            torch.from_numpy(s.astype(np.float32))[None], torch.tensor([1.0])
        )
...
def torch_example_use():
    ...
    if model_type not in model_list:
        print(
            'The model type is not in the list, maybe the name has changes, check torch.hub.list of the repo'
        )
    ...
    with torch.no_grad():
        reps = model(
            wavs
        )  # list of unpadded representations `[rep1, rep2, ...]`, each erp is of the shape `(extracted_seqlen, feature_dim)`
...
def use_tests():
    ...
    df_pContrast[df_pContrast.target_phoneme == 'T'][
        ['pred_gibberish', 'pronounciation_guide']
    ]
    df_pContrast[df_pContrast.target_phoneme == 'D'][
        ['pred_gibberish', 'pronounciation_guide']
    ]
    df_pContrast[df_pContrast.target_phoneme == 'IH0_D'][
        ['pred_gibberish', 'pronounciation_guide']
    ]
