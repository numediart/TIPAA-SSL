...
def determine_start_end_vectors(df_segmented, time_per_output=0.02):
    ...
    df_segmented["start_vector"] = (df_segmented["start"] * (1 / time_per_output)).astype(
        int
    )
    return df_segmented
...
def phone_vectors(
    ...
    phone_type='phones',
    extractor_function=get_last_hidden_state,
    unstressed=True,
):
    df_segmented = determine_start_end_vectors(
        df_segmented, time_per_output=time_per_output
    )
...
def phone_concat_vectors_with_silence(
    s,
    fs,
    df_segmented,
    processor,
...
def instances_per_phoneme(
    df_t,
    processor,
    model,
    number_of_examples=100,
    time_per_output=0.02,
    phone_type='cmu_phone',
    extractor_function=get_last_hidden_state,
    unstressed=True,
):
    average_vectors = []
    ...
    for i in tqdm(range(number_of_examples)):
        ...
        if 'genre' in df_t.iloc[i].keys():
            new_df['genre'] = df_t.iloc[i]['genre']
        if 'speaker' in df_t.iloc[i].keys():
            new_df['speaker'] = df_t.iloc[i]['speaker']
...
def plot_reduction(
    ...
    for i, txt in enumerate(one_example_per_label[legend_label]):
        plt.annotate(
            txt, (one_example_per_label['x'].iloc[i], one_example_per_label['y'].iloc[i])
        )
...
def use_tests():
    ...
    plot_reduction(
        df_all_instances, reduction_technique='umap', base_name='wav2vec_logits'
    )
