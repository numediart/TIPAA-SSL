...
names_symbols_dict={name:symbols_name_df[symbols_name_df.Currency==name].Symbol.tolist() for name in names}
symbols_names_dict={symbol:symbols_name_df[symbols_name_df.Symbol==symbol].Currency.tolist() for symbol in symbols}
...
for s in symbols_names_dict:
    ...
    if len(s)==1:
        if s!="$": #$ is a symbol that is used in regex, so we do it separately, adding a blackslash to escape it
            currency_regexes[s]=[]
...
def normalize_numbers(text):
    ...
    for s in currency_regexes:
        for el in currency_regexes[s]:
            text = re.sub(el, lambda x: _expand_currency(x.group(1),symbols_names_dict[s][0]), text)
