...
def a_test_prefill(base_url='http://localhost:8000', client=app.test_client()):
    ...
    d = call_prefill_for_sentence(
        sentence, mode="MFA_IPA", lang="es_ES", base_url=base_url, client=client
    )
    ...
    d = call_prefill_for_sentence(
        sentence, mode="MFA_IPA", lang="en_GB", base_url=base_url, client=client
    )
    # d=ast.literal_eval(r.data.decode('utf8'))
    sentence = "A 22 heures, j'ai rendez-vous avec le CEO, Indya, et un ingénieur d'une start-up à 300 k dollars, puis avec le CTO !"
    d = call_prefill_for_sentence(
        sentence, mode="MFA_IPA", lang="fr_FR", base_url=base_url, client=client
    )
    # d=ast.literal_eval(r.data.decode('utf8'))
...
def prefill_content_phrases(
    path="data/ST_content_all_phrases_08_02_2023_12_07_13.csv",
    path_db_export="data/query_results-2022-10-26_75652.csv",
):
    ...
    all_sentences_no_duplicates = all_sentences[
        ~all_sentences.sentence.isin(db_phrases.union(db_phrases_lower))
    ]
    all_sentences_no_duplicates = all_sentences_no_duplicates.drop_duplicates(
        ['sentence']
    )
    ...
    syl_errors_df = phonetics_data[
        phonetics_data.apply(lambda r: len(r.n_syl_mismatches) > 0, axis=1)
    ]
    syl_errors_text = syl_errors_df.apply(
        lambda r: [r.segmented_text.split(' ')[i] for i in r.n_syl_mismatches], axis=1
    )
    syl_errors_phonetics = syl_errors_df.apply(
        lambda r: [r.pronounciation_guide_hr.split(' ')[i] for i in r.n_syl_mismatches],
        axis=1,
    )
    syl_errors_methods = syl_errors_df.apply(
        lambda r: [r.used_method_for_syl_text[i] for i in r.n_syl_mismatches], axis=1
    )

    if len(syl_errors_df) > 0:
        pd.DataFrame([syl_errors_text, syl_errors_phonetics, syl_errors_methods]).T

    print('List of inconsistencies in stress:')
    phonetics_data[
        phonetics_data.apply(lambda r: len(r.n_stress_inconsistencies) > 0, axis=1)
    ].apply(
        lambda r: [r.phonetics.split(' ')[i] for i in r.n_stress_inconsistencies], axis=1
    )
...
def request_for_audio_file(
    r, base_url='http://localhost:8000', endpoint='/phonemeContrast', client=requests
):
...
def get_results(
    ...
    for i, r in tqdm(df.iterrows()):
        res = request_for_audio_file(
            r, base_url=base_url, endpoint=endpoint, client=client
        )
        assert res.status_code == 200
...
def test_stress_detection(
    ...
    results_ws, failures_ws = get_results(
        df_test, base_url=base_url, endpoint=route + 'word', client=client
    )

    request_for_audio_file(
        df_test.iloc[0], base_url=base_url, endpoint=route + 'word', client=client
    )
...
def test_audio64(
    ...
    res = request_for_audio_file(
        r, base_url=base_url, endpoint='/v2/w2v/stress/sentence', client=client
    )
    print(res.data)
    assert res.status_code == 200
...
def use_tests():
    ...
    # from test_DL_api import *
    test_actor_recordings(
        base_url='http://localhost:8000', client=requests, n_ex_by_module=10
    )
    ...
    test_actor_recordings(
        base_url='http://35.187.176.184/', client=requests, n_ex_by_module=10
    )
    test_stress_detection(
        base_url='http://35.187.176.184/', client=requests, n_ex_by_module=10
    )
    ...
    test_actor_recordings(
        base_url='https://spdev.flowchase.app/', client=requests, n_ex_by_module=10
    )
    test_stress_detection(
        base_url='http://34.77.240.102:8000', client=requests, n_ex_by_module=10
    )
    ...
    test_audio64(
        path='data/audio_recordings/SS_1_i_would_love_to_go_to_ireland.m4a',
        base_url='http://135.125.247.39:8000',
        client=requests,
    )
    ...
    route = '/v2/w2v/stress/'
    get_results(
        df_sentence_stress.iloc[3:4, :],
        endpoint=route + 'sentence',
        client=app.test_client(),
    )
    request_for_audio_file(
        df_sentence_stress.iloc[3, :],
        endpoint=route + 'sentence',
        client=app.test_client(),
    )
