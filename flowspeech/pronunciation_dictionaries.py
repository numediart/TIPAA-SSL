...
def get_augmented_cmudict():
    ...
    corrections = {
        ...
        'psychoanalyzing': [
            ['S', 'AY0', 'K', 'OW0', 'AE1', 'N', 'AH0', 'L', 'AY2', 'Z', 'IH0', 'NG']
        ],
        'wireless': [['W', 'AY1', 'ER0', 'L', 'IH0', 'S']],
        'teleworker': [['T', 'EH1', 'L', 'AH0', 'W', 'ER2', 'K', 'ER0']],
        'imagesetter': [['IH1', 'M', 'AH0', 'JH', 'S', 'EH2', 'T', 'ER0']],
        'debugged': [['D', 'IY0', 'B', 'AH1', 'G', 'D']],
        'pc': [['P', 'IY0', 'S', 'IY1']],
        'firewall': [['F', 'AY1', 'ER0', 'W', 'AO0', 'L']],
        'uninstalled': [['AH2', 'N', 'IH0', 'N', 'S', 'T', 'AO1', 'L', 'D']],
        'multithreading': [
            ['M', 'AH1', 'L', 'T', 'IY0', 'TH', 'R', 'EH2', 'D', 'IH0', 'NG']
        ],
        'wallonia': [['W', 'AH0', 'L', 'OW1', 'N', 'Y', 'AH0']],
        'eire': [['EH1', 'R', 'AH0']],
        'sovereign': [['S', 'AA1', 'V', 'ER0', 'AH0', 'N']],
        'sovereignty': [['S', 'AA1', 'V', 'ER0', 'AH0', 'N', 'T', 'IY0']],
        'maieutics': [['M', 'EY0', 'UW1', 'T', 'IH0', 'K', 'S']],
        'maieutic': [['M', 'EY0', 'UW1', 'T', 'IH0', 'K']],
        'stonemason': [['S', 'T', 'OW1', 'N', 'M', 'EY0', 'S', 'AH0', 'N']],
        'archbishop': [['AA1', 'R', 'CH', 'B', 'IH0', 'SH', 'AH0', 'P']],
        'striated': [['S', 'T', 'R', 'AY0', 'EY1', 'T', 'IH0', 'D']],
        'myoglobin': [['M', 'AY2', 'AH0', 'G', 'L', 'OW1', 'B', 'IH0', 'N']],
        'orthopaedic': [['AO2', 'R', 'TH', 'AH0', 'P', 'IY1', 'D', 'IH0', 'K']],
        'hypocalcemic': [
            ['HH', 'AY2', 'P', 'OW0', 'K', 'AE0', 'L', 'S', 'EH1', 'M', 'IH0', 'K']
        ],
        'hyperpolarize': [['HH', 'AY2', 'P', 'ER0', 'P', 'OW1', 'L', 'ER0', 'AY2', 'Z']],
        'norepinephrine': [
            ['N', 'AO2', 'R', 'AH0', 'P', 'AH0', 'N', 'EH1', 'F', 'R', 'IH0', 'N']
        ],
        'amputee': [['AE2', 'M', 'P', 'Y', 'AH0', 'T', 'IY1']],
        'baseball': [['B', 'EY1', 'S', 'B', 'AO0', 'L']],
        'downhill': [['D', 'AW0', 'N', 'HH', 'IH1', 'L']],
        'hm': cmudict_dict['hum'],
...
def differs_by_one_insertion(seq1, seq2):
    ...
    if small_seq != big_seq[:inserted_idx] + big_seq[inserted_idx + 1 :]:
        return False, None, None
    ...
    for key, value in my_dict.items():
        ...
        for i in range(len(value)):
            for j in range(i + 1, len(value)):
                is_one_diff, inserted_token, inserted_idx = differs_by_one_insertion(
                    value[i], value[j]
                )
...
def get_formatted_cmudict(phonetic_dict=cmudict_dict, mode='CMU'):
    ...
    df['formatted_phonetics'] = df['syl_p'].apply(
...
def mfa_english_add_schwa_alternative(mfa_d):
    ...
    result_keys, values_small, inserted_idxs = select_keys_with_one_diff(
        cmudict_dict, token="AH0"
    )
...
def generate_acronym_letter_mfa_dicts():
    ...
    with open('data/acronyms_es_ES_mfa.dict', 'w') as f:
        f.write(json.dumps(acronym_dict_long))
    with open('data/acronyms_es_LA_mfa.dict', 'w') as f:
        f.write(json.dumps(acronym_dict_long_LA))
    ...
    with open('data/acronyms_en_GB_mfa.dict', 'w') as f:
        f.write(json.dumps(acronym_dict_long))
    with open('data/acronyms_en_US_mfa.dict', 'w') as f:
        f.write(json.dumps(acronym_dict_long_US))
    ...
    acronym_dict_long['w'] = [
        max(mfa_g2p('doublevé', lang_to_MFA_g2p_models[lang])['doublevé'], key=len)
    ]
    acronym_dict_long['x'] = [['i', 'k', 's']]
    acronym_dict_long['y'] = [
        max(mfa_g2p('igrec', lang_to_MFA_g2p_models[lang])['igrec'], key=len)
    ]
    acronym_dict_long['z'] = [['z', 'ɛ', 'd']]

    with open('data/acronyms_fr_FR_mfa.dict', 'w') as f:
        f.write(json.dumps(acronym_dict_long))
        ...
        for idx_alt in range(len(mfa_us[k])):
            ...
            if len(mfa_us[k][idx_alt]) >= 3:
                ...
                for i in range(len(char_list) - 2):
                    if (
                        char_list[i] in mfa_diphtongs
                        and char_list[i + 1] == 'ɹ'
                        and (char_list[i + 2] not in d['vowels'])
                    ):
            ...
            if len(mfa_us[k][idx_alt]) >= 2:
                if mfa_us[k][idx_alt][-1] == 'ɹ' and (
                    mfa_us[k][idx_alt][-2] in mfa_diphtongs
                ):
                    # print(k, ': ', mfa_us[k])
...
def get_augmented_mfa_dict(lang='es_ES'):
    ...
    with open('data/acronyms_' + lang + '_mfa.dict', 'r') as f:
        acronyms = json.loads(f.read())
    for k in acronyms:
        d[k] = acronyms[k]

    if lang.split('_')[0] == "fr":
        with open('data/add_dict_' + lang + '.dict', 'r') as f:
            add_dict = json.loads(f.read())
        for k in add_dict:
            d[k] = add_dict[k]
    ...
    if lang.split('_')[0] == "en":
        ...
        if lang == "en_US":
            ...
            d["hypothetical"] = [
                ['h', 'aj', 'p', 'ə', 'θ', 'ɛ', 'tʲ', 'ɪ', 'k', 'ɫ̩'],
                ['h', 'aj', 'p', 'ə', 'θ', 'ɛ', 'ɾʲ', 'ɪ', 'k', 'ɫ̩'],
            ]
        ...
        if lang == "en_GB":
            mfa_us = get_mfa_dict(
                path='data/' + lang_to_MFA_g2p_models["en_US"] + '.dict'
            )
...
def add_mfa_dicts():
    ...
    apostroph_s_words[
        "apostroph_s_phone"
    ] = apostroph_s_words.formatted_phonetics.str.split('_').apply(
        lambda r: r[-1].lower()
    )
    lang = "en_GB"
    mfa_d = get_augmented_mfa_dict(lang)
    new_words = apostroph_s_words.apply(
        lambda r: {
            r['text']: [el + [r['apostroph_s_phone']] for el in mfa_d[r['text'][:-2]]]
        }
        if r['text'][:-2] in mfa_d
        else float('nan'),
        axis=1,
    ).dropna()
    ...
    for k in corr_d_dict:
        d_dict[k] = corr_d_dict[k]
    for k in d_dict:
        add_dict[k] = d_dict[k]

    with open('data/add_dict_en_GB.dict', 'w') as f:
        f.write(json.dumps(add_dict))
    ...
    with open('data/add_dict_en_US.dict', 'w') as f:
        f.write(json.dumps(add_dict))
    ...
    for k in mfa_d:
        if mfa_d[k][0][0] in d['vowels']:
            for letter in apostroph_letter_prefixes:
                add_dict["'".join([letter, k])] = [
                    [apostroph_letter_prefixes[letter]] + el for el in mfa_d[k]
                ]

    with open('data/add_dict_fr_FR.dict', 'w') as f:
        f.write(json.dumps(add_dict))
...
def build_mfa_phone_set():
    ...
    with open("data/mfa_phones.json", "w") as outfile:
        outfile.write(json.dumps(phones))
...
with open('data/mfa_phones.json', 'r') as openfile:
    ipa_alphabet = json.load(openfile)
...
cmu_reducer[
    'ʔ'
] = 'T'  # "butter", "uh-oh" sound. UK accent has instances of "T" pronounced like that
...
for lv in long_vowels:
    cmu_reducer[lv] = cmu_reducer[lv[:-1]]
for lv in long_vowels_ascii:
    cmu_reducer[lv] = cmu_reducer[lv[:-1]]

j_consonants = [el for el in ipa_alphabet if el[-1] == 'ʲ']
for jc in j_consonants:
    cmu_reducer[jc] = cmu_reducer[jc[:-1]]
