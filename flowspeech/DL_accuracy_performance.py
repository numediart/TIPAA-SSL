...
def syllable_contrast_for_actor_recordings(model=default_model):
    ...
    np.histogram(
        all_phones_df[
            (all_phones_df.phones == "AA") & (all_phones_df.pred_phones_audio == "AO")
        ].pred_proba
    )
    ...
    np.histogram(
        all_phones_df[
            (all_phones_df.phones == "AO") & (all_phones_df.pred_phones_audio == "AA")
        ].pred_proba
    )
    ...
    for i, r in tqdm(df_pContrast.iterrows()):
        ...
        def inference():
            try:
                phonetics = (
                    df_pContrast[df_pContrast.cmu_phonetics != r.cmu_phonetics]
                    .sample()
        ...
        pred_phones_audio = drop_consecutive_duplicate_elements(
            phonetic_content.pred_phones_audio.tolist()
        )
    ...
    match_rate = 1 - len(pred_df[pred_df.phones != pred_df.pred_phones_audio]) / len(
        pred_df
    )
    ...
    phones = [
        [p]
        for p in sum([p.replace('|', '_').split('_') for p in phonetics.split(' ')], [])
    ]
    ...
    for i, r in tqdm(df.iterrows()):
        ...
        GT_overall_confidence = (
            phonetic_content['GT_proba_means'] * phonetic_content['n_frames']
        ).sum() / sum(phonetic_content.n_frames)
        GT_overall_confidences.append(GT_overall_confidence)
        pred_overall_confidence = (
            phonetic_content['pred_proba_means'] * phonetic_content['n_frames']
        ).sum() / sum(phonetic_content.n_frames)
    ...
    all_phones_df.to_csv(
        'performance_results/actor_recordings_all_predictions_post_correction.csv'
    )
    ################### Data analysis of predictions ################
    all_phones_df = pd.read_csv(
        'performance_results/actor_recordings_all_predictions.csv'
    )
    ...
    syl_indexation_df = all_phones_df[
        ['sentence_idx', 'word_idx', 'syl_idx']
    ].drop_duplicates()
    all_syls_records = []
    print(len(syl_indexation_df))
    for i, r in tqdm(syl_indexation_df.iterrows()):
        syl_df = all_phones_df[all_phones_df.sentence_idx == r.sentence_idx][
            all_phones_df.word_idx == r.word_idx
        ][all_phones_df.syl_idx == r.syl_idx]
        ...
        pred = drop_consecutive_duplicates(
            syl_df[['pred_phones_audio']][syl_df.pred_phones_audio != '[SIL]']
        ).pred_phones_audio.tolist()
        d = {'GT': '_'.join(GT), 'pred': '_'.join(pred)}
        all_syls_records.append(d)
    all_syls_df = pd.DataFrame.from_records(all_syls_records)
    overall_syl_rate = len(all_syls_df[all_syls_df.GT == all_syls_df.pred]) / len(
        all_syls_df
    )
...
def phoneme_confusion_experiments(label='w2v_equilibrated_mailabs_pca_95_knn_10_w'):
    ...
    phoneme_confusions(
        ...
        n=100,
        accent='UK',
        name='plots/vowels_confusions_on_synth_words_' + label,
    )
    phoneme_confusions(
        phonemes=cmu_vowels,
        performance_function=pContrast_on_synth_words,
    ...
    phoneme_confusions(
        ...
        n=100,
        accent='UK',
        name='plots/consonants_confusions_on_synth_words_' + label,
    )
    phoneme_confusions(
        phonemes=cmu_consonants,
        performance_function=pContrast_on_synth_words,
...
def vowels_consonants_confusions_from_audiobook_data(n=100, data_set='test-other'):
    ...
    for c in tqdm(cmu_consonants):
        ...
        try:
            predictions[c], results[c] = pContrast_from_audiobook_data(
                data_set=data_set, target_phones=c, n=n, alternatives=cmu_consonants
            )
        except:
    ...
    for v in tqdm(cmu_vowels):
        ...
        try:
            predictions[v], results[v] = pContrast_from_audiobook_data(
                data_set=data_set, target_phones=v + '1', n=n, alternatives=cmu_vowels
            )
        except:
...
def syl_confusions_on_synth_words():
    ...
    syls = df.syl_p.apply(
        lambda r: set(['_'.join([unstress(p) for p in el]) for el in r])
    ).tolist()
...
def final_ed_confusions_on_synth_words(
    n=100, model=default_model, name='plots/final_ed_synth_words'
):
    df = synth_words_data()
    ...
    df_id = df[
        (df.phonetics.str.endswith('AH0_D') | df.phonetics.str.endswith('IH0_D'))
        & df.text.str.endswith('ed')
    ]
    df_t = df[(df.phonetics.str.endswith('_T')) & df.text.str.endswith('ed')]
    df_d = df[
        (
            ~(df.phonetics.str.endswith('AH0_D') | df.phonetics.str.endswith('IH0_D'))
            & ~df.phonetics.str.endswith('_T')
        )
        & df.text.str.endswith('ed')
    ]
...
def final_s_confusions_on_synth_words(
    n=100, model=default_model, name='plots/final_s_synth_words'
):
    ...
    words_no_s_candidates = set(
        list(df_iz.text.str[:-2]) + list(df_s.text.str[:-1]) + list(df_z.text.str[:-1])
    )
...
def h_sound_confusions_on_synth_words(
    n=100, model=default_model, name='plots/start_end_synth_words'
):
    df = synth_words_data().dropna()
...
def h_sound_artificial_data(model=default_model):
    ...
    tqdm.pandas()
    df_correct['phonetics'] = df_correct.progress_apply(
        lambda r: prefill_for_sentence(r.text)['phonetics'], axis=1
    )
    df_correct['target_word_idx'] = df_correct.apply(
        lambda r: remove_special_characters(r.text.lower())
        .split(' ')
        .index(r['target word'].lower().strip()),
        axis=1,
    )
    df_correct['target_word_indexes'] = df_correct[
        'target_word_idx'
    ]  # .apply(lambda r: [r])

    df_correct_h = df_correct[
        df_correct['rule'] != "without the sound h at the beginning"
    ]
    df_correct_no_h = df_correct[
        df_correct['rule'] == "without the sound h at the beginning"
    ]
    ...
    df_correct_h['target_syllable_idx'] = df_correct_h.apply(
        lambda r: [
            "HH" in el for el in r.phonetics.split(' ')[r.target_word_idx].split('|')
        ].index(True),
        axis=1,
    )
    df_correct_h['target_syllable_indexes'] = df_correct_h['target_syllable_idx']
    df_correct_h.apply(
        lambda r: r.phonetics.split(' ')[r.target_word_idx].split('|'), axis=1
    )

    selections = {'HH': df_correct_h, '': df_correct_no_h}
    results, result_dfs = compute_start_end_confusions_from_selections(
        selections, model=model
    )
    ...
    df_incorrect['phonetics'] = df_incorrect.progress_apply(
        lambda r: prefill_for_sentence(r["expected text"])['phonetics'], axis=1
    )
    df_incorrect['target_word_idx'] = df_incorrect.apply(
        lambda r: remove_special_characters(r["expected text"].lower())
        .split(' ')
        .index(r['target word'].lower().strip()),
        axis=1,
    )
    df_incorrect['target_word_indexes'] = df_incorrect[
        'target_word_idx'
    ]  # .apply(lambda r: [r])
    ...
    df_incorrect['target_syllable_idx'] = df_incorrect.apply(
        lambda r: [
            "HH" in el for el in r.phonetics.split(' ')[r.target_word_idx].split('|')
        ].index(True),
        axis=1,
    ...
    (
        results_incorrect,
        result_dfs_incorrect,
    ) = compute_start_end_confusions_from_selections(selections_incorrect, model=model)

    compute_start_end_confusions_from_selections(
        {'HH': df_incorrect.iloc[-1:]}, model=model
    )

    df_incorrect.index = result_dfs_incorrect['HH'].index
    df_incorrect.loc[
        result_dfs_incorrect['HH'][
            result_dfs_incorrect['HH'].phonetic_detection == "HH"
        ].index.tolist(),
        :,
    ]
...
def final_s_from_artificial_data(model=default_model):
    ...
    df_iz = df[
        df_target_word_p.str.endswith('AH0_Z') | df_target_word_p.str.endswith('IH0_Z')
    ]
    df_s = df[df_target_word_p.str.endswith('_S')]
    df_z = df[
        ~(df_target_word_p.str.endswith('AH0_Z') | df_target_word_p.str.endswith('IH0_Z'))
        & ~df_target_word_p.str.endswith('_S')
    ]

    selections = {'IH0_Z': df_iz, 'S': df_s, 'Z': df_z}
    ...
    for t in targets:
        ...
        success_rate = len(result_dfs[t][result_dfs[t].phonetic_detection == t]) / len(
            result_dfs[t]
        )
    ...
    selections['IH0_Z'].reset_index(drop=True)[
        result_dfs['IH0_Z'].status.str.contains('not')
    ]

    selections['IH0_Z'].reset_index(drop=True)[
        result_dfs['IH0_Z'].phonetic_detection != 'IH0_Z'
    ]
    # d=count_values(phonetic_detections)
...
def pronunciation_aspects_from_audiobook_data(
    n=100, data_set='test-other', model=default_model
):
    _, _, d = start_end_phoneme_from_audiobook_data(
        phoneme='HH', basis='HH', n=n, data_set=data_set
    )
    r = d.T['HH']
    _, _, d = start_end_phoneme_from_audiobook_data(
        phoneme='EH1', basis='HH', n=1000, data_set=data_set
    )
    r = d[d.index != 'HH'].sum()

    # start_end_phoneme_from_audiobook_data(phoneme='AA', n=10000, data_set=data_set)
    _, _, d = start_end_phoneme_from_audiobook_data(
        phoneme='S', basis='S', position="end", n=n, data_set=data_set, model=model
    )
    r = d.T['S']
    _, _, d = start_end_phoneme_from_audiobook_data(
        phoneme='K', basis='S', position="end", n=n, data_set=data_set, model=model
    )
    r = d[d.index != 'S'].sum()
...
def model_comparison():
    ...
    with open('stats_pipeline_basis_it_is_onnx', 'w') as f:
        f.write(stats_pipeline.__str__())
