...
def formatted_audiobook_data(selection, libri_words_df, target_phones=None):
    ...
    selection['cmu_phonetics'] = selection.apply(
        lambda r: ' '.join(
            [
                '|'.join(['_'.join(syl) for syl in SonoriPy(w)[0]])
                for w in r.split_phonetics
            ]
        ),
        axis=1,
    )
    ...
    selection['cmu_phonetics'] = selection.apply(
        lambda r: r['cmu_phonetics'].replace('  ', ' AH0 '), axis=1
    )
    ...
    def find_first(lst, predicate):
        return next((i for i, j in enumerate(lst) if predicate(j)), float('nan'))

    find_first_containing_target = lambda l: find_first(
        l, lambda x: target_phones in x.split('_')
    )

    if target_phones is not None:
        selection['target_syllable_indexes'] = selection.apply(
            lambda r: find_first_containing_target(
                r['cmu_phonetics'].split(' ')[r['word_idx']].split('|')
            ),
            axis=1,
        )
...
def stress_GE_performance_test(level: StressCategory = StressCategory.SENTENCE):
    ...
    if level == 'sentence':
        ...
        df_all['COI_stress_intensities'] = df_all.apply(
            lambda r: r['stress_intensities'][r.ranges[0] : r.ranges[1]], axis=1
        )
        df_all['COI_bins'] = df_all.apply(
            lambda r: r['bins'][r.ranges[0] : r.ranges[1]], axis=1
        )
        df_all['COI_stress_binaries'] = df_all.apply(
            lambda r: r['stress_binaries'][r.ranges[0] : r.ranges[1]], axis=1
        )
        ...
        BOI = df_all.apply(
            lambda r: r['COI_stress_binaries'][r['COI_bins'].index(1)], axis=1
        )
        ...
        word_stress_binaries = word_stress_binaries[
            word_stress_binaries.apply(
                lambda r: sum([(np.prod(el) == 1) & (len(el) > 1) for el in r])
            )
            == 0
        ]
        ...
        def select_by_len(stress_binaries):
            ...
            for l in range(max_len):
                all_word_bins_by_len.append(
                    [el for el in all_word_bins if len(el) == l + 1]
                )
        ...
        word_stress_binaries = word_stress_binaries[
            word_stress_binaries.apply(lambda r: len(r))
            == df['stress_binaries'].apply(lambda r: len(r))
        ]
        df = df.loc[word_stress_binaries.index]

        assert len(sum(word_stress_binaries.tolist(), [])) == len(
            sum(df['stress_binaries'].tolist(), [])
        )
...
def pContrast_for_actor_recordings(
    ...
    if speakers is not None:
        df_pContrast = df_pContrast[
            df_pContrast.audio_file_url.apply(lambda r: r.split('/')[-2]).isin(speakers)
        ]
...
def pContrast_from_audiobook_data(
    ...
    libri_words_df = libri_words_df[
        ~libri_words_df.file_idx.isin(
            libri_words_df[libri_words_df.word == '<unk>'].file_idx.unique()
        )
    ]
    ...
    if target_phones != 'G':
        ...
        selection = libri_words_df[
            libri_words_df.phones.str.endswith(' ' + target_phones)
            | libri_words_df.phones.str.contains(target_phones + ' ')
        ]
    else:
        # same idea, but only for G/NG
        selection = libri_words_df[
            libri_words_df.phones.str.startswith(target_phones + ' ')
            | libri_words_df.phones.str.contains(' ' + target_phones)
        ]
    ...
    df = selection
    df_words = df.apply(
        lambda r: r.cmu_phonetics.split(' ')[r.word_idx].replace('|', '_'), axis=1
    )
    df_target = df[
        (
            df_words.str.endswith('_' + target_phones)
            | df_words.str.startswith(target_phones + '_')
...
def start_end_phoneme_from_audiobook_data(
    phoneme='HH',
    basis='HH',
    contrast="start",
    data_set='dev-clean',
...
def final_ed_from_audiobook_data(
    data_set='test-other', target_phones='D', n=None, model=default_model
):
    libri_words_df = build_librispeech_words_df(data_set=data_set, n=n)
    # there is a tag <unk> when a word is unknown. I filter out the files corresponding to these before performance test
    libri_words_df = libri_words_df[
    ...
    selection = select_libri(
        select_libri(libri_words_df, 'D', option='endswith'),
        'ed',
        column='word',
        option='endswith',
    )
    ...
    selection_t = select_libri(
        select_libri(libri_words_df, 'T', option='endswith'),
        'ed',
        column='word',
        option='endswith',
    )
...
def final_s_from_audiobook_data(data_set='dev-clean', n=None, model=default_model):
    ...
    words_in_s = [
        el
        for el in cmudict_dict.keys()
        if el[-1] == 's'
        and cmudict_dict[el][0][-1] == 'S'
        and el[:-1] in cmudict_dict
        and cmudict_dict[el[:-1]][0][-1] != 'S'
    ]
    ...
    words_in_z = [
        ...
        for el in cmudict_dict.keys()
        if el[-1] == 's'
        and cmudict_dict[el][0][-1] == 'Z'
        and el[:-1] in cmudict_dict
        and cmudict_dict[el[:-1]][0][-1] != 'Z'
    ]
    ...
    selections_no_s['cmu_phonetics'] = selections_no_s.apply(
        lambda r: ' '.join(
            r.cmu_phonetics.split(' ')[: r.word_idx]
            + [r.cmu_phonetics.split(' ')[r.word_idx] + '_S']
            + r.cmu_phonetics.split(' ')[r.word_idx + 1 :]
        ),
        axis=1,
    )
    selections_no_z['cmu_phonetics'] = selections_no_z.apply(
        lambda r: ' '.join(
            r.cmu_phonetics.split(' ')[: r.word_idx]
            + [r.cmu_phonetics.split(' ')[r.word_idx] + '_Z']
            + r.cmu_phonetics.split(' ')[r.word_idx + 1 :]
        ),
        axis=1,
    )
    ...
    result_df_no_s = compute_predictions(
        selections_no_s[:10],
        target_phones='S',
        tech_function=start_end_contrast_from_formatted_phonetics_audio,
        basis='IH0_S',
    ...
    result_df_s = compute_predictions(
        selections_s,
        target_phones='S',
        tech_function=start_end_contrast_from_formatted_phonetics_audio,
        basis='IH0_S',
    ...
    result_df_no_z = compute_predictions(
        selections_no_z[:10],
        target_phones='Z',
        tech_function=start_end_contrast_from_formatted_phonetics_audio,
        basis='IH0_Z',
    ...
    result_df_z = compute_predictions(
        selections_z,
        target_phones='Z',
        tech_function=start_end_contrast_from_formatted_phonetics_audio,
        basis='IH0_Z',
...
def final_ed_fake_mistakes(n=100):
    ...
    df['fpath'] = df['path']
    df_target = df[
        (df.phonetics.str.endswith('_' + "IH0_D")) & ~(df.text.str.endswith('ed'))
    ]
...
def schwa_sound_on_synth_words(
    target_phones='AH0', n=None, accent=None, model=default_model
):
    df = synth_words_data(accent=accent)

    df_target = df[
        (
            df.phonetics.str.endswith('_' + target_phones)
            | df.phonetics.str.startswith(target_phones + '_')
            | df.phonetics.str.contains('_' + target_phones + '_')
        )
    ]
    # just get the index of the first occurence of the target
    df_target['target_syllable_indexes'] = df_target['syl_p'].apply(
        lambda r: [1 if target_phones in el else 0 for el in r].index(1)
    )
    selection = df_target.sample(frac=1, random_state=0)[:n]
    selection['cmu_phonetics'] = selection['phonetics']
    ...
    result_df = compute_predictions(
        selection,
        target_phones=target_phones,
        tech_function=schwa_sound_from_formatted_phonetics_audio,
        model=model,
    )
    phonetic_detections = result_df.phonetic_detection
...
def final_ed_on_synth_words(
    target_phones='IH0_D', n=None, accent=None, model=default_model
):
    df = synth_words_data(accent=accent)

    df_target = df[
        (df.phonetics.str.endswith('_' + target_phones)) & (df.text.str.endswith('ed'))
    ]
    # just get the index of the first occurence of the target
    df_target['target_syllable_indexes'] = -1
...
if False:
    ...
    def syl_contrast_on_synth_words(
        syl_target='P_EH1', n=None, accent=None, model=default_model
    ):
        ...
        selection = df[
            df.apply(lambda r: syl_target.split('_') in r.syl_p, axis=1)
        ].sample(frac=1, random_state=0)[:n]

        selection['target_syllable_indexes'] = selection.syl_p.apply(
            lambda r: r.index(syl_target.split('_'))
        )
        selection['cmu_phonetics'] = selection['phonetics']
        result_df = compute_predictions(
            selection,
...
def use_tests():
    ...
    ds_actor_F1_prod = []
    for p in o_list:
        results_df, d = pContrast_for_actor_recordings(
            target_phones=p, speakers=['F1'], model=prod_model
        )
    ...
    ds_actor_prod = []
    for p in ed_list:
        results_df, d = final_ed_for_actor_recordings(
            target_phones=p, model=default_model_charsiu
        )
        ds_actor_prod.append(d)
    ...
    for p in ed_list:
        results_df, d = final_ed_for_actor_recordings(
            target_phones=p, model=default_model
        )
    ...
    ds_user_prod_ed = []
    for p in ed_list:
        results_df, d = final_ed_for_user_data(
            target_phones=p, model=default_model_charsiu, n=100
        )
    ...
    for p in ed_list:
        results_df, d = final_ed_for_user_data(
            target_phones=p, model=default_model, n=100
        )
    ...
    ds_user_prod_o = []
    for p in o_list:
        results_df, d = pContrast_for_user_data(
            target_phones=p, model=default_model_charsiu, n=100
        )
    ...
    for p in o_list:
        results_df, d = pContrast_for_user_data(
            target_phones=p, model=model_cv_knn, n=100
        )
    ...
    for p in o_list:
        results_df, d = pContrast_for_actor_recordings(
            target_phones=p, speakers=['F1'], model=model
        )
    ...
    for p in o_list:
        results_df, d = pContrast_for_actor_recordings(
            target_phones=p,
            speakers=['M1', 'F4', 'tts', 'F2', 'M4', 'flwc-phrase-audios', 'zoe'],
            model=model,
        )
    ...
    for s in speakers:
        ...
        for p in o_list:
            results_df, d = pContrast_for_actor_recordings(
                target_phones=p, speakers=[s], model=model
            )
