...
def build_df_all_frames(
    ...
    df_all_frames = instances_per_frame(
        df_t,
        processor,
        model,
...
def build_df_all_phoneme_instances(
    ...
    df_all_instances = instances_per_phoneme(
        df_t_train,
        processor,
        model,
        number_of_examples=number_of_examples,
...
def load_dataset_commonvoice(
    ...
    for lang_code in lang_codes:
        df_temp = pd.read_json(
            path + '/{}/{}_{}.json'.format(lang_code, split, phone_set)
        )
        df_temp_metadata = pd.read_csv(
            path + '/{}/{}.tsv'.format(lang_code, split), sep='\t'
        )

        df_temp_metadata = df_temp_metadata.loc[
            df_temp_metadata.path.isin(df_temp.filename + '.mp3')
        ]
...
def load_dataset_MAILABS(lang_codes, path='./data/MAILABS', phone_set='CMU'):
    ...
    for lang_code in lang_codes:
        df_temp = pd.read_csv(
            path + '/MAILABS_shuffled_aligned-{}_{}.csv'.format(lang_code, phone_set)
        )
...
def load_libri_dataset():
    ...
    df_t_train["phone_df"] = df_t_train.apply(
        lambda r: pd.DataFrame.from_records(r.phone_df), axis=1
    )
    df_t_test["phone_df"] = df_t_test.apply(
        lambda r: pd.DataFrame.from_records(r.phone_df), axis=1
    )
...
def load_libri_dataset_audio_timings(df_t_test, number_of_examples=100):
    if "phone" in df_t_test.iloc[0].phone_df.columns:
        df_cmu_phones = df_t_test.apply(
            lambda r: pd.DataFrame.from_records(r.phone_df).phone.tolist(), axis=1
        )
    else:
        df_cmu_phones = df_t_test.apply(
...
def load_test_dataset(df_t_test, number_of_examples=100):
    df_ipa_phones_test = pd.Series(
        df_t_test.phone_df.apply(lambda r: list(r['ipa_phone'].values()))
    )
    test_examples = []
    for N in range(number_of_examples):
        example = df_t_test.iloc[N]
...
def build_df_segmented(df_t, model, forced_aligner, mode):  # mode = "CMU" or "MFA_IPA"
    ...
    for _, row in df_t.iterrows():
        ...
        phone_prob_matrix_nonsil, _, _ = forced_aligner.get_phone_prob_matrix_nonsil(
            phone_prob_matrix
        )
        status, aligned_phones = forced_aligner.get_forced_alignment(
            phone_prob_matrix_nonsil, target_phonemes
        )
        pred_phones_audio, probs_means = forced_aligner.predict(
...
def build_df_segmented_all(
    df_t, model, forced_aligner, mode, language_code
):  # mode = "CMU" or "MFA_IPA"
    ...
    if mode == 'CMU':
        df_segmented.target_phonemes = df_t.phone_df.apply(
            lambda r: list(r['cmu_phone'].values())
        )
    else:
        df_segmented.target_phonemes = df_t.phone_df.apply(
            lambda r: list(r['ipa_phone'].values())
        )
    ...
    for _, r in df_t.iterrows():
        phones.append(
            prefill_for_sentence(sentence=r.text, lang=language_code, mode=mode)[
                'cmu_phonetics'
            ]
        )
    ...
    for _, ex in df_segmented.iterrows():
        ...
        predicted_phones, probs_means = forced_aligner.predict(
            aligned_phones, phone_prob_matrix_nonsil
        )
...
def use_tests():
    ...
    df_all_frames = build_df_all_frames(
        df_t.sample(frac=1, random_state=0), 'phone', number_of_examples=1000
    )
