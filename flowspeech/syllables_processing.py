...
def syllables_data_fr(syl_sep='|'):
    ...
    df.loc[(df.orthosyll.str[:2] == 's-'), 'orthosyll'] = df[
        (df.orthosyll.str[:2] == 's-')
    ].orthosyll.apply(
        lambda my_str: my_str[: my_str.index('-')] + my_str[my_str.index('-') + 1 :]
    )
    df.loc[(df.orthosyll.str[:3] == 'ch-'), 'orthosyll'] = df[
        (df.orthosyll.str[:3] == 'ch-')
    ].orthosyll.apply(
        lambda my_str: my_str[: my_str.index('-')] + my_str[my_str.index('-') + 1 :]
    )
    ...
    df.loc[df.orthosyll.str.contains('-s-'), 'orthosyll'] = df.loc[
        df.orthosyll.str.contains('-s-'), 'orthosyll'
    ].str.replace('-s-', '-s')
    df.loc[df.orthosyll.str.contains('-ch-'), 'orthosyll'] = df.loc[
        df.orthosyll.str.contains('-ch-'), 'orthosyll'
    ].str.replace('-ch-', '-ch')
    df.loc[df.orthosyll.str.contains('-th-'), 'orthosyll'] = df.loc[
        df.orthosyll.str.contains('-th-'), 'orthosyll'
    ].str.replace('-th-', '-th')
    df.loc[df.orthosyll.str.contains('-n-'), 'orthosyll'] = df.loc[
        df.orthosyll.str.contains('-n-'), 'orthosyll'
    ].str.replace('-n-', '-n')
    df.loc[df.orthosyll.str.contains('-j-'), 'orthosyll'] = df.loc[
        df.orthosyll.str.contains('-j-'), 'orthosyll'
    ].str.replace('-j-', '-j')
    df.loc[df.orthosyll.str.contains('-r-'), 'orthosyll'] = df.loc[
        df.orthosyll.str.contains('-r-'), 'orthosyll'
    ].str.replace('-r-', '-r')
    ...
    df_e = df_bad_n_syl[
        (wrong_n_syl_in_text == df_bad_n_syl.nbsyll + 1)
        & (df_bad_n_syl.orthosyll.str[-1] == 'e')
    ]
    df_es = df_bad_n_syl[
        (wrong_n_syl_in_text == df_bad_n_syl.nbsyll + 1)
        & (df_bad_n_syl.orthosyll.str[-2:] == 'es')
    ]
    df_ent = df_bad_n_syl[
        (wrong_n_syl_in_text == df_bad_n_syl.nbsyll + 1)
        & (df_bad_n_syl.orthosyll.str[-3:] == 'ent')
    ]
    ...
    syllables.loc[:, 'syllables'] = syllables.loc[:, 'syllables'].str.replace(
        '-', syl_sep
    )
...
def syllables_data(syl_sep='|'):
    ...
    syllables = (
        syllables.dropna()
    )  # there is one row that is nan... (maybe just a \n at the end of one of the files)
    ...
    # 8032                       as|pired
    # 8642                        at|tire
    syllables_ire = syllables[
        syllables['syllables'].str.lower().str.contains('ire')
        & ~syllables['syllables'].str.lower().str.contains('aire')
        & ~syllables['syllables'].str.lower().str.contains('oire')
    ]
    syllables.loc[syllables_ire.index, 'syllables'] = (
        syllables_ire['syllables'].str.replace('ire', 'i|re').str.replace('Ire', "I|re")
    )

    syllables_ism = syllables[syllables['syllables'].str.lower().str.contains('ism')]
    syllables.loc[syllables_ism.index, 'syllables'] = syllables_ism[
        'syllables'
    ].str.replace('ism', 'i|sm')
    ...
    syllables.loc[syllables_ithm.index, 'syllables'] = syllables_ithm[
        'syllables'
    ].str.replace('ithm', 'i|thm')

    syllables_ythm = syllables[syllables['syllables'].str.lower().str.contains('ythm')]
    syllables.loc[syllables_ythm.index, 'syllables'] = syllables_ythm[
        'syllables'
    ].str.replace('ythm', 'y|thm')
...
def combined_sonoripy(word="dépendanc", orig_word="dépendance", lang="fr_FR"):
    ...
    phonetics = (
        phonetize(orig_word, lang=lang)
        .replace(stress_symbol, "")
        .replace(second_stress_symbol, "")
        .split('_')
    )

    letter_by_syl, sonorities_letters = SonoriPy(
        str_to_list_of_char(word), mode='letters'
    )
    ...
    syl_starts_letters = [
        (aligned_tokens[aligned_tokens.phonemes == syl_start].letters.iloc[0])
        for syl_start in syl_starts
    ]
...
def syllabified_text(
    word,
    n,
    syllables_df=pd.read_csv('data/syllables.csv'),
    lang="en_GB",
    combined_sonoripy_dtw=True,
):
    ...
    try:
        ...
        a = syllables_df[syllables_df.n_syls == syllables_df.n_syls_SonoriPy].loc[
            syllables_df.normalized_text == word
        ]
        if len(a) > 0:
            syls_text = a.syllables.values[0]
        else:
            syls_text = syllables_df[
                syllables_df.normalized_text == word
            ].syllables.values[0]
        used_method = 'dataset'
    except IndexError:
        ...
        vowels, nasals, fricatives, affricates, stops = (
            d['vowels'],
            d['nasals'],
            d['fricatives'],
            d['affricates'],
        ...
        if lang == "en_GB" or lang == "en_US":
            if (
                word[-2:] == "ed"
                and word[-3] not in ['t', 'd']
                and word[-4:] != "ired"
                and word[-3:] != 'led'
            ):  # this last is treated hereafter because it dependes
                word = word[:-2] + 'd'
                modified_ed = True
            ...
            # reduction for e.g. "smiles" (vowel before l), but not for "angles, muscles, articles, ...", and not for "alleles,angeles,anopheles,isosceles"
                ...
                word[-1] == "e" and word[-2:] != 'le'
            ):  # this last one is treated hereafter because it depends
                word = word[:-1]
                trailing_e = True
            elif (
                word[-2:] == "le" and word[-3] not in stops
            ):  # do it for e.g. "smile", but not for "angle, muscle, article, ..."
                word = word[:-1]
                trailing_e = True
...
def n_vowels(word="coffee", lang="en_GB"):
    ...
    if lang.split('_')[0] == "en":
        if word in cmudict_dict and len(cmudict_dict[word]) > 0:
            ...
            n = 0
            for el in phonetics:
                if unstress(el.lower()) in d['vowels']:
                    n += 1
    ...
    for el in phonetics:
        ...
        if (unstress(el.lower()) in d['vowels']) or (
            unstress(el.lower()[0]) in d['vowels']
        ):
            n += 1
    return n
...
def use_tests():
    ...
    syllabified_text(
        word, nv, syllables_df=pd.read_csv('data/syllables_fr_FR.csv'), lang="fr_FR"
    )
    ...
    def plot_SSP_DTW(word):
        ...
        fig, axs = dtwvis.plot_warping(
            ...
            s2,
            path,
            warping_line_options={'linewidth': 0.5, 'color': 'red', 'alpha': 0.8},
        )
